...
describe('MemoryRepository', () => {
  ...
  it('고정비 추가/수정/삭제', async () => {
    ...
    const added = await repo.addFixedCost({
      ...
      amount: 5000, variability: '고정', active: true, sortOrder: 99, yearMonth: '2026-07',
    });
    expect((await repo.listFixedCosts('2026-07'))).toHaveLength(40);
    await repo.updateFixedCost(added.id, { amount: 6000 });
    const found = (await repo.listFixedCosts('2026-07')).find((f) => f.id === added.id);
